import { ActionRowBuilder, ButtonBuilder, ButtonStyle, CommandInteraction, ComponentType, EmbedBuilder, EmbedField, InteractionReplyOptions, Message, ModalBuilder, TextInputBuilder, TextInputStyle, User } from "discord.js";
...
export class Battle {
    ...
    constructor({ users, interaction, details, time }: {
        users: [ User, User ];
    ...
    private get clocksField(): [EmbedField, EmbedField] {
        ...
        return [
            {
                name: this.users[0].username ?? "Premier joueur",
                value: this.turn % 2 === 0 ? `<t:${((Date.now() + this.clocks[0]) / 1000).toFixed(0)}:R>` : secondsToWeeks(this.clocks[0] / 1000),
                inline: true
            },
            {
                name: this.users[1].username ?? "Deuxième joueur",
                value: this.turn % 2 === 1 ? `<t:${((Date.now() + this.clocks[1]) / 1000).toFixed(0)}:R>` : secondsToWeeks(this.clocks[1] / 1000),
                inline: true
            }
        ]
    }
    ...
    private embed() {
        const embed = new EmbedBuilder()
            ...
            .setDescription(`C'est à <@${this.userToPlay.id}> de jouer\n\nRésolvez ce calcul \`\`\`${this.currentCalcul.calcul}\`\`\``)
            .setColor('Orange')
            .setFields(this.clocksField)
            .setTimestamp()
            .setFooter({ text: "Bataille de problèmes", iconURL: this.userToPlay.displayAvatarURL({ forceStatic: false }) })
        return embed;
    }
    ...
    private onSolve() {
        this.interaction.client.on('buttonInteraction', async(button) => {
            ...
            if (button.customId === 'resign') {
                const confirmationMsg = await button.reply({
                    ephemeral: true,
                ...
                this.end('resign', this.users.find(x => x.id !== button.user.id));
                return;
            }
    ...
    private addPoints() {
        const points = Math.floor(this.points[this.users.indexOf(this.users.find(x => x.id === this._winner.id))] * 80 / 100);
    ...
    private get errorEmbed() {
        return new EmbedBuilder()
            .setTitle("Erreur")
            .setDescription(`<@${this.users.find(x => x.id !== this._winner.id).id}> s'est trompé\n\`\`\`${this.currentCalcul.calcul} = ${this.currentCalcul.result}\`\`\`\n\n<@${this._winner.id}> est le gagnant`)
            .setColor('#00ff00')
            .setTimestamp()
            .setFields([...this.clocksField, { name: "Points", value: `${this.pointsWinned.toLocaleString()} points` }])
    }
    private get resignEmbed() {
        return new EmbedBuilder()
            .setTitle("Abandon")
            .setDescription(`<@${this.users.find(x => x.id !== this._winner.id).id}> a abandonné\n\nLe gagnant est <@${this._winner.id}>`)
            .setColor('#00ff00')
            .setTimestamp()
            .setFields([...this.clocksField, { name: "Points", value: `${this.pointsWinned.toLocaleString()} points` }])
    }
    private get timeEmbed() {
        return new EmbedBuilder()
            ...
            .setDescription(`<@${this.users.find(x => x.id !== this._winner.id).id}> a perdu au temps\n\nLe gagnant est <@${this._winner.id}>`)
            .setColor('#00ff00')
            .setTimestamp()
            .setFields([...this.clocksField, { name: "Points", value: `${this.pointsWinned.toLocaleString()} points` }])
    }
