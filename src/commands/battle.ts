...
export default new AmethystCommand({
    ...
    const dot = [
        { x: DotLength.OneOne, y: 'un et un' },
        { x: DotLength.TwoOne, y: 'deux et un' },
        { x: DotLength.TwoTwo, y: 'deux et deux' },
        { x: DotLength.ZeroOne, y: 'zéro et un' },
        { x: DotLength.ZeroTwo, y: 'zéro et deux' }
    ];
    const length = [
        { x: NumberLength.FourFour, y: 'quatre et quatre' },
        { x: NumberLength.ThreeFour, y: 'trois et quatre' },
        { x: NumberLength.ThreeThree, y: 'trois et trois' },
        { x: NumberLength.ThreeTwo, y: 'trois et deux' },
        { x: NumberLength.TwoFour, y: 'deux et quatre' },
        { x: NumberLength.TwoTwo, y: 'deux et deux' }
    ];
    const calcType = [
            ...
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        `<@${
                            interaction.user.id
                        }> vous défie dans une bataille de calculs. Relevez-vous le défi ?\n\n**Modalités :**\nTemps : ${
                            time.name
                        } ( ${time.time} minutes plus ${time.increment * 60} secondes )\nNombres : \`${
                            numbersType === NumbersType.Digit ? 'décimaux' : 'entiers'
                        }\`${
                            numbersType === NumbersType.Integer
                                ? ''
                                : `\nNombres après la virgule : ${dot.find((x) => x.x === dotLength).y}`
                        }\nLongueur des nombres : ${length.find((x) => x.x === numbersLength).y}\nCalculs : ${
                            calcType.find((x) => x.x === operation).y
