...
export default new AmethystCommand({
    ...
    const map = (award: award, type: awardType) => {
        ...
        return `${type} : ${
            award ? `<@${award.userId}> avec ${award.amount} calculs en ${award.seconds} secondes` : 'Non défini'
        }`;
    };
    ...
    const embed = new EmbedBuilder()
        .setTitle('Records')
        .setDescription(
            `Voici les records de vitesse de calcul :\n${Object.keys(awards)
                .map((k) => map(awards[k], k as awardType))
                .join('\n')}`
        )
        .setColor('Orange')
        .setTimestamp();
